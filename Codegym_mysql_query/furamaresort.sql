...
where (nv.ho_ten like 'H%' or nv.ho_ten like 'K%' or nv.ho_ten like 'T%') and char_length(ho_ten) <=15;

-- 3. Hiển thị thông tin của tất cả khách hàng có độ tuổi từ 18 đến 50 tuổi và có địa chỉ ở “Đà Nẵng” hoặc “Quảng Trị”.
select *, ROUND(DATEDIFF(CURDATE(),ngay_sinh)/365,0) as Tuổi from khach_hang kh
where (ROUND(DATEDIFF(CURDATE(),ngay_sinh)/365,0) >=18 && ROUND(DATEDIFF(CURDATE(),ngay_sinh)/365,0) <= 50) && (dia_chi like '%Đà Nẵng%' or dia_chi like '%Quảng Trị%');

-- 4.Đếm xem tương ứng với mỗi khách hàng đã từng đặt phòng bao nhiêu lần. Kết quả hiển thị được sắp xếp tăng dần theo số lần đặt phòng của khách hàng. 
-- Chỉ đếm những khách hàng nào có Tên loại khách hàng là “Diamond”.
select kh.ma_khach_hang, kh.ho_ten, count(ma_hop_dong) as so_lan_dat_phong
...
select kh.ma_khach_hang, kh.ho_ten, lk.ten_loai_khach, hd.ma_hop_dong, dv.ten_dich_vu, hd.ngay_lam_hop_dong, hd.ngay_ket_thuc, (ifnull(dv.chi_phi_thue + sum(ifnull(hdct.so_luong*dvdk.gia,0)),0)) as tong_tien
...
group by kh.ma_khach_hang, hd.ma_hop_dong,hd.ngay_lam_hop_dong
order by kh.ma_khach_hang;

-- 6. Hiển thị ma_dich_vu, ten_dich_vu, dien_tich, chi_phi_thue, ten_loai_dich_vu của tất cả các loại dịch vụ 
-- chưa từng được khách hàng thực hiện đặt từ quý 1 của năm 2021 (Quý 1 là tháng 1, 2, 3).
select dv.ma_dich_vu, dv.ten_dich_vu, dv.dien_tich, dv.chi_phi_thue, ldv.ten_loai_dich_vu
from dich_vu dv right join loai_dich_vu ldv on dv.ma_loai_dich_vu = ldv.ma_loai_dich_vu
where dv.ma_dich_vu
not in (select dv.ma_dich_vu
from hop_dong hd right join dich_vu dv on hd.ma_dich_vu = dv.ma_dich_vu 
where quarter(hd.ngay_lam_hop_dong) =1&& year(hd.ngay_lam_hop_dong)>=2021) ;

-- 7. Hiển thị thông tin ma_dich_vu, ten_dich_vu, dien_tich, so_nguoi_toi_da, chi_phi_thue, ten_loai_dich_vu 
-- của tất cả các loại dịch vụ đã từng được khách hàng đặt phòng trong năm 2020 nhưng chưa từng được khách hàng đặt phòng trong năm 2021.
select distinct dv.ma_dich_vu, dv.ten_dich_vu, dv.chi_phi_thue,ldv.ten_loai_dich_vu
from hop_dong hd  join dich_vu dv on hd.ma_dich_vu = dv.ma_dich_vu 
				  join loai_dich_vu ldv on dv.ma_loai_dich_vu = ldv.ma_loai_dich_vu
where (dv.ma_dich_vu
not in (select dv.ma_dich_vu
from hop_dong hd right join dich_vu dv on hd.ma_dich_vu = dv.ma_dich_vu 
where  year(hd.ngay_lam_hop_dong)=2021)) && year(hd.ngay_lam_hop_dong) =2020 ;
...
select month(hd.ngay_ket_thuc) as thang, count(kh.ma_khach_hang) as so_luong_khach_hang
from khach_hang kh join hop_dong hd on kh.ma_khach_hang = hd.ma_khach_hang
where year(hd.ngay_ket_thuc) = 2021
group by month(hd.ngay_ket_thuc)
order by month(hd.ngay_ket_thuc) asc;
...
where lk.ten_loai_khach='Diamond' &&( kh.dia_chi like '%Vinh%' or kh.dia_chi like '%Quảng Ngãi%')
...
select hd.ma_hop_dong, nv.ho_ten as ho_ten_nhan_vien, kh.ho_ten as ho_ten_khach_hang , kh.so_dien_thoai, dv.ten_dich_vu, ifnull(sum(hdct.so_luong),0) as so_luong_dich_vu_di_kem , hd.tien_dat_coc
from khach_hang kh left join hop_dong hd on kh.ma_khach_hang = hd.ma_khach_hang
				   left join nhan_vien nv on hd.ma_nhan_vien = nv.ma_nhan_vien
                   left join dich_vu dv on hd.ma_dich_vu = dv.ma_dich_vu
                   left join hop_dong_chi_tiet hdct on hd.ma_hop_dong=hdct.ma_hop_dong
where ((month(hd.ngay_lam_hop_dong) in (10,11,12)) && year(hd.ngay_lam_hop_dong) = 2020) && dv.ma_dich_vu 
		not in (select dv.ma_dich_vu from hop_dong hd join dich_vu dv on hd.ma_dich_vu=dv.ma_dich_vu
									 where (month(hd.ngay_lam_hop_dong) in (1,2,3,4,5,6)) && year(hd.ngay_lam_hop_dong) = 2021)
group by hd.ma_hop_dong;
...
-- having so_luong_dich_vu_di_kem = (select sum(hdct.so_luong) as so_luong_dich_vu_di_kem
-- 								  from khach_hang kh join hop_dong hd on kh.ma_khach_hang = hd.ma_khach_hang
-- 													join hop_dong_chi_tiet hdct on hd.ma_hop_dong = hdct.ma_hop_dong
-- 													join dich_vu_di_kem dvdk on hdct.ma_dich_vu_di_kem = dvdk.ma_dich_vu_di_kem
-- 								  group by dvdk.ma_dich_vu_di_kem
...
select hd.ma_hop_dong,  dv.ten_dich_vu, ldv.ten_loai_dich_vu,count(dvdk.ma_dich_vu_di_kem) as so_lan_su_dung
from hop_dong hd join dich_vu dv on hd.ma_dich_vu = dv.ma_dich_vu
                 join loai_dich_vu ldv on dv.ma_loai_dich_vu = ldv.ma_loai_dich_vu 
                 join hop_dong_chi_tiet hdct on hd.ma_hop_dong = hdct.ma_hop_dong
                 join dich_vu_di_kem dvdk on hdct.ma_dich_vu_di_kem = dvdk.ma_dich_vu_di_kem
...
delete from nhan_vien where ma_nhan_vien in (select ma_nhan_vien from (  select nv.ma_nhan_vien from nhan_vien nv where nv.ma_nhan_vien not in(
																		 select nv.ma_nhan_vien
																		from nhan_vien nv join hop_dong hd on nv.ma_nhan_vien = hd.ma_nhan_vien
																		where (year(hd.ngay_lam_hop_dong) between 2019 and 2021)
																		group by nv.ma_nhan_vien
																		having count(hd.ngay_lam_hop_dong) > 0)) as temp);
...
delete from hop_dong_chi_tiet where ma_hop_dong_chi_tiet in ( select ma_hop_dong_chi_tiet from( select hdct.ma_hop_dong_chi_tiet from khach_hang kh join hop_dong hd on kh.ma_khach_hang = hd.ma_khach_hang
											join hop_dong_chi_tiet hdct on hd.ma_hop_dong =  hdct.ma_hop_dong
...
where ma_khach_hang in ( select ma_khach_hang from( select kh.ma_khach_hang
													from khach_hang kh join hop_dong hd on kh.ma_khach_hang = hd.ma_khach_hang
													where year(hd.ngay_lam_hop_dong) <2021) as temp);
...
select nv.ma_nhan_vien, nv.ho_ten as tenNV,  nv.email as emailNV,  nv.so_dien_thoai as sdtNV,  nv.ngay_sinh as nsNV, "NV" as loai
from nhan_vien nv 
union
select kh.ma_khach_hang, kh.ho_ten as tenKH,  kh.email as emailKH,  kh.so_dien_thoai as sdtKH,  kh.ngay_sinh as nsKH, "KH" as loai
from khach_hang kh;
...
-- watashi
create view v_nhan_vien as (select nv.*, count(hd.ma_hop_dong) as so_hop_dong from nhan_vien nv join hop_dong hd on nv.ma_nhan_vien = hd.ma_nhan_vien
...
-- 26. Tạo Trigger có tên tr_cap_nhat_hop_dong khi cập nhật ngày kết thúc hợp đồng, cần kiểm 
-- tra xem thời gian cập nhật có phù hợp hay không, với quy tắc sau: Ngày kết thúc hợp đồng 
-- phải lớn hơn ngày làm hợp đồng ít nhất là 2 ngày. Nếu dữ liệu hợp lệ thì cho phép cập nhật,
--  nếu dữ liệu không hợp lệ thì in ra thông báo “Ngày kết thúc hợp đồng phải lớn hơn ngày làm 
-- hợp đồng ít nhất là 2 ngày” trên console của database.
-- Lưu ý: Đối với MySQL thì sử dụng SIGNAL hoặc ghi log thay cho việc ghi ở console.

-- 27. Tạo Function thực hiện yêu cầu sau:
-- Tạo Function func_dem_dich_vu: Đếm các dịch vụ đã được sử dụng với tổng tiền là > 2.000.000 VNĐ.
-- Tạo Function func_tinh_thoi_gian_hop_dong: Tính khoảng thời gian dài nhất tính từ lúc bắt đầu
...
-- dịch vụ đó (tức là xóa các bảng ghi trong bảng dich_vu) và xóa những hop_dong sử dụng dịch
 -- vụ liên quan (tức là phải xóa những bản gi trong bảng hop_dong) và những bản liên quan khác.
